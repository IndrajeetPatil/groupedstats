...
As demonstrated here, the benefit of formatted summary output is that, it can be
directly fed into other routines (e.g., preparing a plot of `mean` and `sd`
values in `ggplot2`).

Another thing to note about `grouped_summary` function is that if no `measures`
are specified, the function will compute summary for all variables of the
...
variables across multiple levels of grouping variable(s). For example, we can
use the `gapminder` dataset to study two relationships of interest for **each country** 
across years:
...
```{r grouped_robustlm, warning = FALSE, messsage = FALSE}
library(gapminder)
library(dplyr)
options(tibble.width = Inf)            # show me all columns
